...
if __name__ == '__main__':
    ...
    savepath = './models/social_lstm/data/'    
    filenames = (
        [ # train
    ...
    for filename in filenames[0]:        
        ...
        with open(savepath + 'train/UCY/' + filename + '.txt', 'w') as f:
            for ped in range(rawdata.num_pedestrians):
                for frame in range(rawdata.num_steps):
                    if(rawdata.mask_p[frame, ped] == 1):
                        f.write(f'{frame} {ped} {rawdata.position[frame, ped, 1]} {rawdata.position[frame, ped, 0]}\n')
    
    for filename in filenames[1]:        
        rawdata = RawData()
        rawdata.load_trajectory_data(filepath + filename + '.npy')

        with open(savepath + 'validation/UCY/' + filename + '.txt', 'w') as f:
            for ped in range(rawdata.num_pedestrians):
                for frame in range(rawdata.num_steps):
                    if(rawdata.mask_p[frame, ped] == 1):
                        f.write(f'{frame} {ped} {rawdata.position[frame, ped, 1]} {rawdata.position[frame, ped, 0]}\n')
    ...
    for filename in filenames[2]:        
        ...
        with open(savepath + 'test/UCY/' + filename + '.txt', 'w') as f:
            for ped in range(rawdata.num_pedestrians):
                for frame in range(rawdata.num_steps):
                    if(rawdata.mask_p[frame, ped] == 1):
                        f.write(f'{frame} {ped} {rawdata.position[frame, ped, 1]} {rawdata.position[frame, ped, 0]}\n')
