...
class VNBatchNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        norm = norm.unsqueeze(2) # [B, N_feat, 1, N_samples, ...]
        norm_bn = norm_bn.unsqueeze(2) # [B, N_feat, 1, N_samples, ...]
        x = x / norm * norm_bn # norm is rotational equivariant
...
class VNMaxPool(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        d = self.map_to_dir(x.transpose(1,-1)).transpose(1,-1)
        dotprod = (x*d).sum(2, keepdims=True)  # [B, N_feat, N_samples, ...]
        idx = dotprod.max(dim=-1, keepdim=False)[1] #[B, N_feat, N_samples] / [B, N_feat]
...
class VNStdFeature(nn.Module):
    def __init__(self, in_channels, dim=4, normalize_frame=False, share_nonlinearity=False, negative_slope=0.2):
        ...
        if normalize_frame:
            ...
            self.vn_lin = nn.Linear(in_channels//4, 1, bias=False)
...
class VNStdFeature_mod(nn.Module):
    def __init__(self, in_channels, out_channels, dim=4, share_nonlinearity=False, negative_slope=0.2):
        super(VNStdFeature_mod, self).__init__()
        self.dim = dim
        # self.normalize_frame = normalize_frame
        self.proj = VNLinear(in_channels, out_channels)
        ...
        # self.vn2 = VNLinearLeakyReLU(in_channels//2, in_channels//4, dim=dim, share_nonlinearity=share_nonlinearity, negative_slope=negative_slope)
        self.vn_lin = nn.Linear(in_channels, 1, bias=False)
